...
func main() {
	...
	// Per-phase wall-time breakdown — emitted UNCONDITIONALLY (= even
	// with --quiet the daemon-driven production runs produce this
	// single-line summary). Sums across chunk workers; with N parallel
	// workers the wall total stacks, so divide by Workers for an
	// apples-to-apples per-chunk view. Bumper-heavy channels (RTL/
	// ProSieben with 100+ templates) should show bumper as a dominant
	// share; bumper>40 % is the threshold to bump --bumper-stride.
