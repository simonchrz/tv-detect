...
type Opts struct {
	...
	SpotLPW           float64              // Gewicht der Spot-Fingerprint-Evidenz im HSMM-Dekoder (Log-Bonus je bekannter Spotgrenze, s. SpotBoundaryLP). 0 = aus (Vorgabe). Wirkt NUR unter --decoder hsmm*: Form() hat keinen Zugriff. Per Kanal ueber detect-config einschalten, sobald ein A/B am versiegelten Satz einen Gewinn zeigt -- dieselbe Regel wie beim BoundarySnap.
	SpotAnchors       []signals.SpotAnchor // bekannte Werbespots dieser Aufnahme (tv-recorder cluster-anchored). Daten, keine Einstellung -- hier, weil sie sonst positional durch sieben Aufrufer geschleift werden muessten. nil = keine Evidenz.
